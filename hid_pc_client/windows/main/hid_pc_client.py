...
CONFIG_PATH = Path(__file__).with_name("hid_client_config.json")
...
class ClipboardSerialClient:
    ...
    def _session(self, ser: serial.Serial) -> None:
        ...
        self._last_local_text = WindowsClipboard.get_text()
...
def main() -> None:
    if not (hasattr(ctypes, "windll") and hasattr(ctypes.windll, "user32")):
        logger.error("This software runs only on Windows.")
        return
